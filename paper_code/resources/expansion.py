...
def Expand_U(model: fasttext.FastText._FastText,
             algorithm: object,
             seed_set_features: list,
             seed_set_label: list,
             expansion_features: list,
             expansion_set_labels: list,
             test_set_features: list,
    ...
    seed_TK = copy.deepcopy(seed_set_features)
    ...
    for i in range(1, len(M)):
        ...
        exp_TK = expansion_features[M[i-1]:M[i]]
        ...
        guesses = small_model.predict(test_set_features)

        _acc.append(accuracy_score(test_set_labels, guesses))
        _precision.append(precision_score(test_set_labels, guesses))
        _recall.append(recall_score(test_set_labels, guesses))
        _f1.append(f1_score(test_set_labels, guesses))
        _seed_size.append(len(seed_labels))

    report = np.array([_seed_size,_acc,_precision,_recall,_f1]).T
    return seed_TK, seed_labels, exp_TK_certain, exp_TK_certain_labels,report
